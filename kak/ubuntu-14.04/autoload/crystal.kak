...
addhl -group / regions -default code crystal       \
    double_string '"' (?<!\\)(\\\\)*"        '' \
    single_string "'" (?<!\\)(\\\\)*'        '' \
    backtick      '`' (?<!\\)(\\\\)*`        '' \
    regex         '/' (?<!\\)(\\\\)*/[imox]* '' \
...
addhl -group /crystal/double_string fill string
addhl -group /crystal/double_string regions regions interpolation \Q#{ \} \{
addhl -group /crystal/double_string/regions/interpolation fill meta

addhl -group /crystal/single_string fill string

addhl -group /crystal/backtick fill meta
addhl -group /crystal/backtick regions regions interpolation \Q#{ \} \{
addhl -group /crystal/backtick/regions/interpolation fill meta

addhl -group /crystal/regex fill meta
addhl -group /crystal/regex regions regions interpolation \Q#{ \} \{
addhl -group /crystal/regex/regions/interpolation fill meta

addhl -group /crystal/comment fill comment

addhl -group /crystal/literal fill meta

addhl -group /crystal/code regex \b([A-Za-z]\w*:(?=[^:]))|([$@][A-Za-z]\w*)|((?<=[^:]):[A-Za-z]\w*[=?!]?)|([A-Z]\w*|^|\h)\K::(?=[A-Z]) 0:identifier
...
%sh{
    ...
    # Highlight keywords
    printf %s "
        addhl -group /crystal/code regex \b(${keywords})\b 0:keyword
        addhl -group /crystal/code regex \b(${attributes})\b 0:attribute
        addhl -group /crystal/code regex \b(${values})\b 0:value
        addhl -group /crystal/code regex \b(${meta})\b 0:meta
    "
}
...
def -hidden _crystal_indent_on_char %{
    eval -no-hooks -draft -itersel %{
        ...
        try %{ exec -draft <a-x> <a-k> ^ \h * (end)        $ <ret> <a-\;> <a-?> ^ ( ( [^\n] * do(\h * \| [^\n] * \| \h * $)? ) | ( \h * (begin|case|for|if|unless|until|while|((private|protected) \h +)?(abstract \h +)?(class|def|struct|macro|module)) ) ) <ret> s \A | \Z <ret> \' <a-&> }
        # try %{ exec -draft <a-x> <a-k> ^ \h * (end)        $ <ret> <a-\;> <a-?> ^ \h * (begin|case|(private \h +)?(abstract \h +)?(class|struct)|((private|protected) \h +)?def|do|for|if|macro|module|unless|until|while) <ret> s \A | \Z <ret> \' <a-&> }
    }
}
...
hook global WinSetOption filetype=crystal %{
    ...
    #hook window InsertChar \n -group crystal-insert _crystal_insert_on_new_line

    alias window alt crystal-alternative-file
}

hook -group crystal-highlight global WinSetOption filetype=(?!crystal).* %{ rmhl crystal }
